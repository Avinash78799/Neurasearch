...
export default function Visualizer({ data, title = "Data Visualization & Analysis" }) {
  ...
  return (
    <div className="rounded-xl border border-[var(--border-primary)] shadow-sm overflow-hidden animate-slide-up bg-[var(--bg-card)] text-[var(--text-primary)]">
      {/* Header */}
      <div className="flex flex-wrap items-center justify-between px-5 py-3.5 border-b border-[var(--border-primary)] gap-2 bg-[var(--bg-secondary)]">
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 rounded-lg bg-[var(--bg-card)] flex items-center justify-center border border-[var(--border-primary)] text-[var(--text-primary)]">
          ...
          <div className="flex p-0.5 rounded-lg bg-[var(--bg-card)] border border-[var(--border-primary)]">
            <button
              ...
              className={`p-1.5 rounded text-xs font-medium transition-all ${
                chartType === "bar" ? "bg-[var(--text-primary)] text-[var(--bg-primary)]" : "text-[var(--text-muted)] hover:text-[var(--text-primary)]"
              }`}
              title="Bar Chart"
            >
            ...
            <button
              ...
              className={`p-1.5 rounded text-xs font-medium transition-all ${
                chartType === "line" ? "bg-[var(--text-primary)] text-[var(--bg-primary)]" : "text-[var(--text-muted)] hover:text-[var(--text-primary)]"
              }`}
            ...
            <button
              ...
              className={`p-1.5 rounded text-xs font-medium transition-all ${
                chartType === "pie" ? "bg-[var(--text-primary)] text-[var(--bg-primary)]" : "text-[var(--text-muted)] hover:text-[var(--text-primary)]"
              }`}
              ...
              return (
                <div key={idx} className="bg-[var(--bg-secondary)] rounded-lg p-3 border border-[var(--border-primary)] space-y-1">
                  ...
                  <div className="flex items-baseline justify-between">
                    ...
                    <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded ${delta >= 0 ? "bg-emerald-500/10 text-emerald-500" : "bg-red-500/10 text-red-500"}`}>
                      {delta >= 0 ? `+${delta}%` : `${delta}%`}
                    </span>
                  </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-around gap-4 py-2">
            <div className="relative w-32 h-32 flex items-center justify-center">
              <svg className="w-full h-full -rotate-90" viewBox="0 0 36 36">
                <circle cx="18" cy="18" r="15.915" fill="transparent" stroke="var(--border-primary)" strokeWidth="3" />
                <circle
                  cx="18"
                  cy="18"
                  r="15.915"
